...
{
    printf("Managed Parallel k-means start\n");
    ...
    while ((delta_global > THRESHOLD) && (iter_counter < MAX_ITER)) //+1 is for the last assignment to cluster centroids (from previous iter)
    {
        ...
        #pragma acc parallel loop private(min_dist) copyin(iter_centroids_global[0:((MAX_ITER + 1) * K_global * 3)], data_points_global[0:(number_of_points_global * 3)]) copy(cluster_points_sum[0:(K_global * 3)], points_inside_cluster_count[0:K_global], point_to_cluster_id[0:number_of_points_global])
    ...
    */
    ...
    #pragma acc parallel loop copyout(data_point_cluster_global[0:number_of_points_global*4]) copyin(data_points_global[0:number_of_points_global*3], point_to_cluster_id[0:number_of_points_global])
    for (i = 0; i < number_of_points_global; i++)
    {
